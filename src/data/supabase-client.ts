...
function ensureConfigured(): void {
  if (!isSupabaseConfigured) {
    throw new Error(
      ...
      'Define VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY en el archivo .env'
    )
  }
}

/** Cliente Supabase singleton (lazy — solo se crea al usarlo) */
let _supabase: SupabaseClient | null = null
export function getSupabaseClient(): SupabaseClient {
  if (!_supabase) {
    ensureConfigured()
    _supabase = createSupabaseClient(SUPABASE_URL!, SUPABASE_ANON_KEY!, {
      auth: {
...
export async function createClient(client: ClientInput): Promise<Client> {
  const { data, error } = await supabase
    .from('clients')
    .insert(camelToSnake(client) as any)
    .select()
    .single()
...
export async function createDepartment(dept: DepartmentInput): Promise<Department> {
  const { data, error } = await supabase
    .from('departments')
    .insert(camelToSnake(dept) as any)
    .select()
    .single()
...
export async function createCenter(center: CenterInput): Promise<Center> {
  const { data, error } = await supabase
    .from('centers')
    .insert(camelToSnake(center) as any)
    .select()
    .single()
...
export async function createTopic(topic: TopicInput): Promise<Topic> {
  const { data, error } = await supabase
    .from('topics')
    .insert(camelToSnake(topic) as any)
    .select()
    .single()
...
export async function createAttachment(
  ...
  const { data, error } = await supabase
    .from('document_attachments')
    .insert(camelToSnake(attachment) as any)
    .select()
    .single()
...
export async function createDocumentVersion(
  ...
  const { data, error } = await supabase
    .from('document_versions')
    .insert(camelToSnake(version) as any)
    .select()
    .single()
...
export async function createAlarm(alarm: AlarmInput): Promise<Alarm> {
  const { data, error } = await supabase
    .from('alarms')
    .insert(camelToSnake(alarm) as any)
    .select()
    .single()
